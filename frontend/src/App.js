    ...
    if (excelFile !== null) {
      ...
      for (let i = 0; i < data.length; i++) {
        const requestBody = {
          ...
          AccountLevel: data[i].AccountLevel,
          ParentAgency: data[i].ParentAgency,
          Agency: data[i].Agency,
          PublishedDate: data[i].PublishedDate,
          PlannedCloseDate: data[i].PlannedCloseDate,
          ExtendedCloseDate: data[i].ExtendedCloseDate,
        };
        ...
        axios
          .post(`http://localhost:8081/upload/excel`, requestBody)
          .then((response) => {
            // alert("Data Uploaded!");
